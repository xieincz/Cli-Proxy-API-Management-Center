...
const DEFAULT_CODEX_TIMEOUT_SECONDS = 30;
...
export function QuotaSection<TState extends QuotaStatusState, TData>({
  ...
  const [codexTimeoutSeconds, setCodexTimeoutSeconds] = useLocalStorage(
  ...
  const handleCodexRefreshAll = useCallback(async () => {
    if (config.type !== 'codex' || disabled || sectionLoading || filteredFiles.length === 0) return;
    const timeoutSeconds = Math.min(600, Math.max(5, Math.round(codexTimeoutSeconds || 30)));
    setCodexTimeoutSeconds(timeoutSeconds);
    setBatchProgress({ completed: 0, total: filteredFiles.length });
    try {
      await loadQuota(filteredFiles, setLoading, {
        concurrency: 10,
  ...
  }, [codexTimeoutSeconds, config.type, disabled, filteredFiles, loadQuota, sectionLoading, setCodexTimeoutSeconds, setLoading]);
          ...
          {config.type === 'codex' && (
            <div className={styles.codexBatchControls}>
              <Input
                ...
                value={String(codexTimeoutSeconds)}
                onChange={(event) => {
                  const value = Number(event.currentTarget.value);
                  if (Number.isFinite(value)) setCodexTimeoutSeconds(value);
                }}
              ...
              <Button
                variant="secondary"
                size="sm"
                onClick={() => void handleCodexRefreshAll()}
